...
在前面的源码分析中对 TiFlash 的计算层和存储层都进行了深入的分析，其中 [TiFlash DeltaTree 存储引擎设计及实现分析 (Part 1)](https://pingcap.feishu.cn/wiki/wikcnIw0lVRfEpsHdbYqdvnSBEg)   [TiFlash DeltaTree 存储引擎设计及实现分析 (Part 2)](https://pingcap.feishu.cn/wiki/wikcnpZCBS3yg9zl0ROHWd7ZPVt) 对 TiFlash 存储层的读写流程进行了完整的梳理，如果读者没有阅读过这两篇文章，建议阅读后再继续本文的阅读。
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f5445c523129489a82f1fc46c24d7aaf~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e561015fdd174acf842e1613c3bba87b~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/7628fea9202f4d03b83f5fa211216815~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/fcdd987148f349708ddd7d5e93a4db1e~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/67e57c397ece475e947e11ba17001b00~tplv-k3u1fbpfcp-zoom-1.image)

这里继续按照上述的方法比较，可以计算得到当前节点的右子树最小的 row_id 为 8，大于要查找的 row_id，因此接下来需要继续遍历当前节点的左子树。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/46287ec046b34f6393f63befaf2f9a22~tplv-k3u1fbpfcp-zoom-1.image)

这里已经遍历到叶子节点，那么这个叶子节点就是我们要查找的可能包含 row_id 为 7 的最右侧的叶子节点。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/5a6398089ba542f4a8b306b36410185b~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/a7a2fdd837204dad9c28144982b541d4~tplv-k3u1fbpfcp-zoom-1.image)

所以我们在写入 DTFile 时必须保证相同主键的数据会写入同一个 Pack，这样在经过 MinMax 索引过滤后才不会发生上述例子的异常情况。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/0d271acde0a345fe91e67fdbdabb199d~tplv-k3u1fbpfcp-zoom-1.image)
